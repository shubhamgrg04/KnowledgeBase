...
var REPLACEMENT_START_RE = `<span\ class="(${escapeRegex(
  HIGHLIGHT_CLASS
)}|${escapeRegex(
  DELETED_CLASS
)})"\ style="background\-color:\ [a-z]+;"(\ data\-highlight\-id="[0-9]+")?>`;
...
function highlight(selString, container, selection, color, highlightIndex) {
  ...
  if (!alreadyHighlighted) {
    ...
    content = content
      .replace(startRe, replacements.end)
      .replace(endRe, replacements.start);
